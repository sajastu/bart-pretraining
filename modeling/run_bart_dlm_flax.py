...
class FlaxDataCollatorForBartDenoisingLM:
    ...
    def __call__(self, examples: List[Dict[str, List[int]]], debug=False) -> BatchEncoding:
        ...
        if self.permute_sentence_ratio > 0.0:
            batch["input_ids"], batch["input_labels"] = self.permute_sentences(batch["input_ids"], batch["input_labels"])
            do_permute = True
        ...
        if self.mask_ratio:
            ...
            # )
            batch["input_ids"], batch["labels"] = self.mask_salient_tokens(
                batch["input_ids"], batch["labels"], batch["input_labels"], do_permute
            )
    ...
    def permute_sentences(self, input_ids, input_labels):
        """
        Shuffle sentences in each document.
        """
        results = input_ids.copy()
        results_input_labels = input_labels.copy()
        ...
        for i in range(input_ids.shape[0]):
            ...
            for j in ordering:
                ...
                sentence_labels = input_labels[i, (sentence_ends_map[i][j - 1] if j > 0 else 0) : sentence_ends_map[i][j]]
                results[i, index : index + sentence.shape[0]] = sentence
                results_input_labels[i, index : index + sentence_labels.shape[0]] = sentence_labels
                index += sentence.shape[0]

        return results, results_input_labels
    ...
    def mask_salient_tokens(self, input_ids, labels, input_labels, do_permute):
        """
        ...
        mask_1d = (np.where(input_labels ==1, 1, 0))
        if mask_1d.sum() > num_tokens_to_mask:
            num_tokens_should_delete = (np.where(input_labels >=1, 1, 0)).sum() - num_tokens_to_mask
            token_probs = np.random.dirichlet(np.random.randint(1, 10, (mask_1d.shape[1])), mask_1d.shape[0]) * mask_1d
            cut_off_th = np.sort(token_probs[token_probs>0])[:num_tokens_should_delete][-1]
            mask_1d = np.where(mask_1d > cut_off_th, mask_1d, 0)
        mask = mask_1d

        if mask.sum() < num_tokens_to_mask:
            num_tokens_remaining = num_tokens_to_mask - mask.sum()

            # generate a sufficient number of span lengths
            span_lengths = np.random.poisson(lam=self.poisson_lambda, size=(num_tokens_remaining,))
            while np.cumsum(span_lengths, 0)[-1] < num_tokens_remaining:
                span_lengths = np.concatenate(
                    [span_lengths, np.random.poisson(lam=self.poisson_lambda, size=(num_tokens_remaining,))]
                )
            ...
            # trim to about num_tokens_to_mask tokens
            cutoff_idx = np.argmin(np.abs(np.cumsum(span_lengths, 0) - num_tokens_remaining)) + 1
            span_lengths = span_lengths[:cutoff_idx]

            # randomly choose starting positions for masking
            is_token_mask = ~(input_ids == self.tokenizer.pad_token_id) & ~special_tokens_mask_inputs & ~mask
...
def generate_batch_splits(samples_idx: np.ndarray, batch_size: int, tokenized_dataset=None, drop_last=True) -> np.ndarray:
    """Generate batches of data for a specified batch size from sample indices. If the dataset size is not divisible by
    the batch size and `drop_last` is `True`, the last incomplete batch is dropped. Else, it is returned."""
    num_samples = len(samples_idx)

    # if tokenized_dataset is not None:
    #     # drop samples whose input_labels are all 1!
    #     tokenized_datasets_tmp = []
    #     for idx, t in tqdm(enumerate(tokenized_dataset)):
    #         if sum(t['input_labels']) != len(t['input_labels']):
    #             tokenized_datasets_tmp.append(t)
    #     tokenized_dataset = tokenized_datasets_tmp.copy()
    #     # free up the mem!
    #     del tokenized_datasets_tmp
    #     samples_idx = np.random.permutation(np.arange(len(tokenized_dataset)))
    #     num_samples = len(samples_idx)
...
def main():
    ...
    def sentence_split_function(example):
        # sents = sentence_tokenizer.tokenize(' '.join(sum(example["text"], [])))
        ...
        sents = example["text_tokens"]
        # use pad token as end of sentence indicator
        # import pdb;pdb.set_trace()
        new_text = [tokenizer.bos_token] + sum([s + [tokenizer.pad_token] for s in sents], [])[:-1] + [tokenizer.eos_token]
        example['annotation'] = [0] + sum([s + [0] for s in example['annotation']], [])[:-1] + [0]
        return {"text": new_text, "input_labels": example['annotation']}
    ...
    def tokenize_function(examples):
        def tokenize_and_preserve_labels(examples):
            ...
            for example_text, example_text_labels in zip(examples[text_column_name], examples["input_labels"]):
                ret_loop_text = []
                ret_loop_labels = []
                bad_example = False
                for word, label in zip(example_text, example_text_labels):
                    ...
                    if len(word_ids) < 1024:
                        ret_loop_text.extend(word_ids)
                        # Add the same label to the new list of labels `n_subwords` times
                        # labels.extend([label] * n_subwords)
                        ret_loop_labels.extend([label] * n_subwords)
                    else:
                        print('Bad example: Longer than 1024: ')
                ...
                if not bad_example:
                    if sum(ret_loop_text) > 0 and sum(ret_loop_labels) != len(ret_loop_text):
                        ret["input_ids"].append(ret_loop_text)
                        ret["input_labels"].append(ret_loop_labels)
                    else:
                        print('Bad example; not added')
                        print(example_text)
                        print(ret_loop_labels)

            return ret

        return BatchEncoding(tokenize_and_preserve_labels(examples))
        # return tokenizer(examples[text_column_name], add_special_tokens=False, return_attention_mask=False)
        # return tokenizer([' '.join(s) for s in examples[text_column_name]], add_special_tokens=False, return_attention_mask=False)

    print()
    print('Toeknizing dataset...')
    ...
    def group_texts(examples):
        ...
        # if 10 consecutive ids then drop
        result = {}
        for je, exam in enumerate(examples['input_ids']):
            last_seen_ids = exam[1:11]
            if len(set(last_seen_ids)) <= 3:
                del examples['input_ids'][je]
                del examples['input_labels'][je]
            else:
                last_seen_ids = []
                for id in exam[11:]:
                    if len(last_seen_ids) < 10:
                        last_seen_ids.append(id)
                    else:
                        if len(set(last_seen_ids)) <= 3:
                            del examples['input_ids'][je]
                            del examples['input_labels'][je]
        ...
        #         result[k].append([t[i : i + max_seq_length]])
        #
        for re, r in enumerate(result['input_labels']):
            if sum(r) == len(r):
                print(result['input_ids'][re])
    ...
    def train_step(state, batch, dropout_rng):
        ...
        def loss_fn(params):
            ...
            batch.pop("input_labels")
            # jax.debug.print(batch["input_ids"])
            ...
            # jax.debug.print('------------')
            # jax.debug.print('batch_idx {} \n input_ids [{}] \n logits {}', batch_idx, batch["input_ids"], logits)
            # jax.debug.print('------------')
            # take average
        ...
        # new_state = new_state.to_fp32(new_state.params)
    ...
    # Define eval fn
    def eval_step(params, batch, batch_idx):
        # jax.debug.print('prior_batch_idx {} \n prior_input_ids {}', batch_idx, batch["input_ids"])
        ...
        # jax.debug.print('------------')
        # jax.debug.print('\n')
        # jax.debug.print('Input ids {}', batch["input_ids"])
        # jax.debug.print('------------')
        # jax.debug.print('\n')
        ...
        # summarize metrics
        metrics = {"loss": loss, "accuracy": accuracy.sum(), "normalizer": label_mask.sum()}
        metrics = jax.lax.psum(metrics, axis_name="batch")
        # jax.debug.print('loss: {}', metrics["loss"])
        return metrics
    ...
    for epoch in epochs:
        ...
        # print(train_samples_idx)
        # print(train_samples_idx.shape)
        train_batch_idx = generate_batch_splits(train_samples_idx, train_batch_size, tokenized_dataset=tokenized_datasets["train"])
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            # new_samples = []
            # for sidx, s in zip(batch_idx, samples):
            #     s['batch_idx'] = sidx
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                for i, batch_idx in enumerate(tqdm(eval_batch_idx, desc="Evaluating ...", position=2)):
                    ...
                    metrics = pad_shard_unpad(p_eval_step, static_return=True)(
                        state.params, model_inputs.data, batch_idx, min_device_batch=per_device_eval_batch_size
                    )
